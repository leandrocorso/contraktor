...
import { getContract, updateContract, deleteContract, contractClearMessages } from '../../redux/contracts/actions';
...
function ContractsEdit(props) {
  ...
  useEffect(() => {
    if (contractReducer && Object.keys(contractReducer).length) {
      const partsId = contractReducer.parts && contractReducer.parts.map(item => item.id) || [];
      const contractParts = contractReducer.parts && contractReducer.parts.map(item => ({
        ...item,
      ...
      const contractData = {
        ...contractReducer,
        title: contractReducer.title,
        effectiveDate: dateToBr(contractReducer.effectiveDate),
        expirationDate: dateToBr(contractReducer.expirationDate),
        // Relationship settings
        partsId,
        parts: contractParts,
        // File upload settings
        href: `${DOWNLOAD_URL}/${contractReducer.filename}`,
        target: '_blank',
      };
  ...
  const handleDeleteDialog = ()=> {
    dispatch(dialogOpen({
      ...
      content: 'Tem certeza que deseja excluir o contrato?',
      submitText: 'SIM',
  ...
  const sendData = () => {
    ...
    dispatch(updateContract(action));
  };
  ...
  || partsIsLoading ? (
    ...
    <div>
      ...
      <form onSubmit={handleSubmit(sendData)}>
        <MyGrid container spacing={4} className={classes.container}>
          ...
          <MyGrid item xs={12} sm={6} md={4} lg={12}>
            ...
            <IndexList items={contract.parts} notFound="Não há partes envolvidas para este contrato, selecione abaixo" />

            <RelationshipList name="partsId" items={parts} selected={initialValues.partsId} onChange={handleChange} />
          </MyGrid>
      ...
      <MyDialog 
        title="Excluir contrato" 
        text="Tem certeza que quer excluir este contrato?" 
